...
import pickle
import os
file=open('/home/cel/iit_kgp_event_extraction_model/fixed_length_input/Beng_training_and_vocab_for_trigger_15_11.pkl','rb')
...
for k in range(len(y_total)):
    try:
        tag_flag='_'.join(y_total[k].split('_')[:-1])#because event trigger tag is of BIO notation
    except:
        tag_flag=y_total[k]
    if tag_flag not in uniq_event_type:
        uniq_event_type.append(tag_flag)
...
with tf.name_scope("embedding"):
    w_embedding = tf.Variable(tf.constant(0.0, shape=[35906, embed_length]), trainable=True, name="w_embedding")
    embedding_placeholder = tf.placeholder(tf.float32, [35906, embed_length])
...
layer_fc=create_fc_layer(output_concat, no_input = 1300, no_output =32)
#print(layer_fc)
...
MODEL_PATH = os.path.join('/home/cel/iit_kgp_event_extraction_model/bengali_trigger_model/40/','my-model.ckpt-50')
...
output_path='/home/cel/iit_kgp_event_extraction_model/output/test_trigger_output/'#output path
...
with tf.Session() as sess:
    ...
    for filename  in os.listdir(path):
        ...
        if os.path.isfile(fullPath):
            ...
            t=0
            while t<len(tagset):
                f_out.write(word_list[t]+'  '+str(uniq_event_type[tagset[t]])+'\n')
                t+=1
            f_out.close()
            test_file.close()
